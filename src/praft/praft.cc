...
//   // The Configuration is restored from the snapshot and log files when the data in the replication group is not
//   empty.
...
//   return {0, "OK"};
// }
butil::Status PRaft::Init(const std::string& group_id, braft::Configuration& init_conf) {
  assert(!node_);

  braft::NodeOptions node_options;
  node_options.initial_conf = init_conf;
  node_options.fsm = this;
  node_options.node_owns_fsm = false;
  node_options.snapshot_interval_s = 0;
  auto prefix = fmt::format("local://{}{}/{}", g_config.db_path.ToString(), "_praft", db_id_);
